...
parser.add_argument('--data_dir', required=True, help='ENsiRNA dataset dir (containing train_*.csv, valid_*.csv)')
parser.add_argument('--output_dir', default=os.path.join(repo_root, 'data'), help='Output directory for converted data')
...
def process_fold(k):
    ...
    def convert_split(df, split_name):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc=split_name):
            ...
            np.save(os.path.join(sirna_dir, f'{rna_idx}.npy'), get_embedding(anti_seq_21))
            np.save(os.path.join(mrna_dir, f'{rna_idx}.npy'), get_embedding(mrna_window))
